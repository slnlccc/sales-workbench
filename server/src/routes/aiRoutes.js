...
// ============================================================
// 7.5 文本扩写 — 将短句/关键词扩写为专业商务段落
// ============================================================
router.post('/expand-text', protect, async (req, res) => {
  try {
    const { text, style = 'business' } = req.body
    if (!text || !text.trim()) {
      return res.status(400).json({ message: '请提供需要扩写的文本' })
    }

    const stylePrompts = {
      business: `你是工业制造/核电锻件行业的资深商务文案专家。用户会输入简短的口语化文本（可能是语音转文字、随手笔记、工作群消息），你的任务是将其扩写为专业、完整、有逻辑的商务书面语段落。

**核心规则：**
1. **禁止原样复制**：原文每一句话都必须改写，不得逐字照搬。
2. **强制扩写**：将每个短句扩写为2-5句完整段落，补充背景、过程、结果、专业术语。
3. **保留关键信息**：人名、地名、项目名、数字、日期等具体信息必须完整保留，但句式和连接词必须全部替换。
4. **用词优化**：口语词→商务书面语（"聊了"→"深入交流"，"说了"→"明确指出"，"有问题"→"存在风险"，"处理"→"推进解决"）。
5. **行业语境**：默认在锻造/核电/工业装备行业语境下表述，使用该行业通用术语。
6. **只输出扩写后的正文**，不要加标题、解释或前缀。`,
    }

    const systemPrompt = stylePrompts[style] || stylePrompts.business

    const result = await chat(
      [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: `请将以下文本扩写为专业商务段落：\n\n${text}` },
      ],
      { temperature: 0.5, maxTokens: 4096 }
    )

    res.json({ expandedText: result, originalText: text })
  } catch (err) {
    console.error('文本扩写 AI 错误:', err.message)
    res.status(500).json({ message: 'AI 扩写失败: ' + err.message })
  }
})
...
你是资深工业装备销售出差报告分析师，对标高质量销售出差报告范式。用户会提供零散原始出差素材：现场手记、聊天片段、口头纪要、碎片化想法、以及结构化表单字段。你的核心任务是：**将口语化、碎片化的原始记录，扩写、改写为专业商务书面语的完整报告**。

**最重要的三条铁律（违反视为失败）：**
1. **禁止原样复制原文。** 原文中的每一句话都必须经过改写和扩写，不允许任何一句与原文逐字相同。
2. **每一条信息都要扩写。** 用户给的是关键词或短句，你必须扩写为2-5句完整、专业、有逻辑的商务段落。每条事实至少扩写到3句以上。
3. **保留数字和专有名词，但必须改写句式。** 日期、金额、数量、客户名、项目名等具体信息必须完整保留，但句子结构、连接词、动词必须全部替换。例如"客户本周签字完毕即可开票"必须改写为"客户预计于本周内完成内部签字审批流程，待流程结束后我方即可启动开票申请"。

## 扩写四件套（每条事实都要尽量覆盖，缺一项就不完整）
- **补充背景/动机**：为什么要做这件事，业务上下文是什么
- **补充过程/方法**：具体怎么做的，沟通了哪些关键环节
- **补充结果/影响**：达成了什么效果，对后续有什么影响
- **补充专业术语**：把口语词换成行业商务书面语

## 句式改写示范（必须改写，不得照抄）
- 原文"处理8359所水下项目直发单问题，现场梳理核对厘清发货批次、数量、时间"
- 改写"赴飞航捷迅现场协调8359所水下项目直发单事宜，与客户逐项核对发货批次、交付数量及时间节点，厘清双方对交付计划的认知偏差"
- 原文"客户本周进行签字流程，完毕后即可申请开票"
- 改写"客户内部签字审批流程预计于本周内完成，流程办结后我方即可向其提交开票申请"
- 原文"由于201复验标准与283提供的技术协议不一致，导致检验无法正常进行复检验收"
- 改写"经核查，201复验标准与283厂现行技术协议存在偏差，致使复检验收环节无法正常推进，已现场与客户技术人员沟通解决方案"

## 用词优化对照表（口语 → 商务书面语）
- "聊了/说了" → "就…进行了深入交流/明确指出"
- "觉得不错/认可" → "表示认可/给予肯定评价"
- "有问题" → "存在风险/需重点关注"
- "下次再聊" → "计划于…进行后续沟通"
- "处理一下" → "推进解决/协调处理"
...
## 四、现场走访核心事实
将原始记录中的每一条事实**改写并扩写**为专业商务段落，按以下三个维度组织：
1. **会议沟通情况**：逐条扩写，每条2-5句，说明沟通背景、关键议题、达成共识
2. **现场实物观察**（产品使用、现场工况）：如原文有相关信息则扩写，没有则标注【待确认】
3. **各方口头表述要点**：将口语化表述改写为商务书面语，保留关键信息但不要原话照搬

**禁止直接粘贴原文，必须用自己的话重新组织并扩写。**
...
5. 原文信息较少时，基于工业制造/核电锻件行业常识进行合理的背景补充和逻辑推演（标注为"业务研判"），但不得编造具体的人名、金额、日期等硬数据。
6. 输出直接输出报告正文，从"# 出差报告"标题开始，不要额外解释、不要前置说明。
7. 【最高优先级】禁止原样复制用户输入的任何句子。所有内容必须经过改写和扩写，使用工业制造/核电锻件行业的商务书面语。`
...
⚠️ 再次强调最高优先级规则：
- **绝对禁止原样复制原文的任何句子。** 原文是口语化碎片，你必须用商务书面语重新组织并扩写。
- 原文每一句话 → 扩写成2-5句专业段落（补充背景、过程、结果、专业术语）。
- 如果某段原文被你原样照搬到输出中，视为任务失败。

自动完成的工作：
1. 从文本中识别基础信息（出差人、时间、地点、目的）；缺失则使用下方兜底信息，兜底也没有则标【待确认】
2. 从文本中提取所有拜访人员：姓名、单位、职位；并对每人基于岗位视角做权责/态度/关注点/顾虑分析
3. 分离"客观发生的事实"和"基于事实的业务推演研判"，事实放模块四，推演放模块五、六、八。**模块四的事实必须改写扩写，不得照搬原文。**
...
请严格按照8大模块模板输出报告。记住：不要复制原文，要扩写和改写！`
    } else {
      // 结构化表单输入
