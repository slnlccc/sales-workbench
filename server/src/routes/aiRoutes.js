...
7. 使用 Markdown 格式输出，报告整体长度 1000-2500 字为宜。`
    ...
    let userContent = ''
    if (d && d.rawText && String(d.rawText).trim().length > 20) {
      userContent = `
您是一位资深的销售管理专家，请阅读以下用户粘贴的"原始出差记录（自由文本）"，
从中**自动提取、分类、归纳、扩充**出结构化信息，并严格按照上方的模板结构输出
一份完整的"出差日报"。

需要您自动完成的工作：
1. **识别基础信息**：从文本中找出"出差人、日期、地点、客户单位、客户姓名/职位"等；
   若文本中没有，则用基本信息字段里填写的值做兜底（如果也没有就填"/"）。
2. **识别出差目的**：从文本中判断属于（获取商机/洽谈订单/维护关系/技术交流/收款/处理问题）哪些项。
3. **归纳八大卡片关键信息**：将文本中的内容归纳到：
   - 行业核心变量 → 关键影响下的 4 小点（标准切换、时间节点、采购模式、远期增量）
   - 锻件市场 → 标杆落地、准入门槛、细分品类
   - 板材市场 → 行业标杆、准入门槛、竞品梯队、我方切入路径
   - 其他客户单位情况（如辅机厂、质量问题/交付问题/框架协议等）
   - 大小业主交流记录
   - 其他收获/行业盈利格局判断
   - 风险（分类）
   - 求助
   - 下一步行动计划（按"序号|事项|责任人|时间"表格化）
4. **自动扩充**：对于关键词/短句，扩写成 2-5 句专业完整段落（不编造人名/公司/日期/数据）。
5. **严格按上方模板输出**：缺少的子标题保留骨架并填"/"。
6. 关键子标题（标准切换、时间节点、采购模式、远期增量、标杆落地、准入门槛、
   细分品类、行业标杆、竞品梯队、我方切入路径、行业盈利格局）必须**全部出现**，
   哪怕只有"/"。

---
【基本信息兜底（如果原始文本中找不到，请以此为准）】
出差人：${d.travelers || '（未提供）'}
出差日期：${d.travelDate || new Date().toISOString().split('T')[0]}
出差地点：${d.location || '（未提供）'}
出差目的（用户勾选）：${d.purpose || '（未勾选，自动识别）'}
客户单位/姓名/职位（用户填写）：${d.clients || '（未提供，自动识别）'}
---
